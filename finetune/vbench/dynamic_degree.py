...
class DynamicDegree:
    ...
    def get_score2(self, imgs, flos):
        assert imgs.shape[0] == flos.shape[0]
        assert len(imgs.shape) == len(flos.shape)
        assert len(imgs.shape) == 4
        max_rads = []
        for i in range(imgs.shape[0]):
            img = imgs[i].permute(1,2,0)
            flo = flos[i].permute(1,2,0)
            ...
            h, w = rad.shape
            rad_flat = rad.flatten()
            cut_index = int(h*w*0.2)

            max_rad = torch.mean(torch.abs(torch.sort(-rad_flat)[0])[:cut_index])
            max_rads.append(max_rad)

        return torch.stack(max_rads)
    ...
    def infer2(self, videos):
        b, c, t, h, w = videos.shape
        scale = min([h, w])
        self.params = {"thres":6.0*(scale/256.0), "count_num":round(4*(t/16.0))}
        static_score = []
        videos_list = torch.unbind(videos, dim=2)
        for image1, image2 in zip(videos_list[:-1], videos_list[1:]):
            assert len(image1.shape) == len(image2.shape)
            padder = InputPadder(image1.shape)
            image1, image2 = padder.pad(image1, image2)
            _, flow_up = checkpoint.checkpoint(self.model, image1, image2, 20, None, True, True)
            # _, flow_up = self.model(image1, image2, iters=20, test_mode=True)
            max_rad = self.get_score2(image1, flow_up)
            static_score.append(max_rad)
        motion_score = torch.mean(torch.stack(static_score,dim=1))/scale
        return motion_score
