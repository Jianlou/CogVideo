...
class DiffRewardModel(object):
    def __init__(self,
        ...
        if reward_type == "HPSv2":
            ...
            self.diff_resize = torchvision.transforms.Resize(224, interpolation=BICUBIC)
            self.diff_crop = torchvision.transforms.CenterCrop(224)
            self.diff_randomcrop = torchvision.transforms.RandomCrop(224)

            
            if cfg.get('hps_21', False):
    ...
    def reward_scorer(self, captions,
                            ...
                            t_round):
        # print(captions)
        # print(denoised_x0.shape)
        # print(torch.max(denoised_x0))
        # print(torch.min(denoised_x0))
        # print(torch.median(denoised_x0.flatten()))
        # assert False
        ### Video data pre-processing
        # video_data.clamp_(0, 1)
        video_num, _, frame_num, _, _ = denoised_x0.shape
        # print("denoised_x0.shape")
        # print(denoised_x0.shape)
        video_data = rearrange(denoised_x0, 'b c f h w -> (b f) c h w')
        ...
        # video_data_resize = self.diff_randomcrop(video_data)
        ...
        # print(pre_captions_chunk[0].shape)
        # print("here here")
        ...
        reward_scores = []
        # print(len(pre_video_chunk))
        # print(len(pre_captions_chunk))
        ...
        for chunk_idx, (vid_chunk, cap_chunk) in enumerate(zip(pre_video_chunk, pre_captions_chunk)):
            ...
            # print(cap_chunk.shape)
            # print(vid_chunk.shape)
            # cap_chunk = cap_chunk.repeat(vid_chunk.shape[0],1)
            # print(cap_chunk.shape)
            # assert False
            outputs = self.reward_model(vid_chunk, cap_chunk)
            # outputs = checkpoint.checkpoint(self.reward_model, vid_chunk, cap_chunk)
