...
class DiffRewardModel(object):
    def __init__(self,
        ...
        if reward_type == "HPSv2":
            model_name = "ViT-H-14"
            if self.cfg.reward_precision == 'fp16':
                precision = 'fp16'
            else:
                precision = 'amp'

            print(f'reward {reward_type}:{model_name}')
            assert os.path.exists('../models/.cache/') and os.path.isdir('../models/.cache/'),"文件夹不存在"
            ...
            self.diff_crop = torchvision.transforms.CenterCrop(224)

            
            if self.cfg.get('hps_21', False):
                checkpoint = torch.load('../models/HPS_v2.1_compressed.pt',map_location='cpu')
                print("load HPS_v2.1_compressed")
            else:
                checkpoint = torch.load('../models/HPS_v2.pt',map_location='cpu')
                print("load HPS_v2")
        ...
        self.selection_method = self.cfg.selection_method
        ...
        if self.reward_normalization:
            self.reward_tracker = PerPromptStatTracker(buffer_size = self.cfg.reward_tracker['buffer_size'],
                                                    min_count = self.cfg.reward_tracker['min_count'])
        if 'mean' in self.cfg.temporal_reward_type:
            if self.reward_normalization:
                self.motion_mean_tracker = PerPromptStatTracker(buffer_size = self.cfg.reward_tracker['buffer_size'],
                                                        min_count = self.cfg.reward_tracker['min_count'])
        if 'std' in self.cfg.temporal_reward_type:
            if self.reward_normalization:
                self.motion_std_tracker = PerPromptStatTracker(buffer_size = self.cfg.reward_tracker['buffer_size'],
                                                        min_count = self.cfg.reward_tracker['min_count'])

        self.adv_clip_max = self.cfg.adv_clip_max
        self.adv_clip_min = - self.cfg.adv_clip_max
        self.reward_weights = self.cfg.reward_weights
        self.ST_reward_weights = self.cfg.ST_reward_weights
        self.step_count = 0
        self.motion_rep = self.cfg.motion_rep
        print(f'Motion representation: {self.motion_rep}')
        self.low_penal_threshold = self.cfg.low_penal_threshold 
    ...
    def reward_scorer(self, captions,
        ...
        chunk_size = min(self.cfg.chunk_size, video_data.shape[0])
        ...
        video_data_resize = self.diff_crop(video_data_resize)
        ...
        _,ff,_,_,_ = pre_video_data.shape
        pre_video_data = rearrange(pre_video_data, 'b f c h w -> (b f) c h w')
        # print(chunk_size)
        # print(pre_video_data.shape)
        pre_video_chunk = torch.chunk(pre_video_data, pre_video_data.shape[0]//chunk_size, dim=0)
        # pre_video_chunk = torch.chunk(pre_video_data, pre_video_data.shape[0]//1, dim=0)
        # for eachck in pre_video_chunk:
        #     print(eachck.shape)
   
        ### Text data pre-processing
        pre_captions = self.tokenizer(captions)
        pre_captions = pre_captions.unsqueeze(1).expand(-1,ff,-1)
        pre_captions = rearrange(pre_captions, 'b ff d -> (b ff) d')
        pre_captions = pre_captions.to(self.device)
        # print(captions)
        # print(pre_captions.shape)
        pre_captions_chunk = torch.chunk(pre_captions, pre_captions.shape[0]//chunk_size, dim = 0)
        # for eachck in pre_captions_chunk:
        #     print(eachck.shape)
        
        if self.exponential_TSN:
            TSN_coef = torch.exp(-torch.abs(global_index - frame_num//2) * self.lambda_TAR)
            # print(TSN_coef.shape)
            # TSN_coef_chunk = torch.chunk(TSN_coef, TSN_coef.shape[0]//chunk_size, dim = 0)
            # TSN_coef_chunk = torch.chunk(TSN_coef, TSN_coef.shape[0]//1, dim = 0)
            # for eachck in TSN_coef_chunk:
            #     print(eachck.shape)
        # assert False
        ...
        with torch.cuda.amp.autocast(enabled=self.cfg.use_fp16):
            for chunk_idx, (vid_chunk, cap_chunk) in enumerate(zip(pre_video_chunk, pre_captions_chunk)):
                ...
                # print(logits_per_image.shape)
                diag = torch.diagonal(logits_per_image) # s, k
                # print(diag)
                # print(diag.shape)
                reward_scores.append(diag)
        diag = torch.cat(reward_scores)
        diag = rearrange(diag, '(b ff) -> b ff',ff=ff)
        reward_scores = torch.mean(diag*TSN_coef, dim=1)
        # print(diag)
        # print(TSN_coef)
        # print(reward_scores)
        # assert False
...
if __name__ == "__main__":
    ...
    "temporal_reward_type": [],\
    "adv_clip_max": 5,\
    "motion_rep": None,\
    "low_penal_threshold": 0.05,\
    "reward_weights": {'reward': 1,'reg': 1},\
    "temp_dir": 'workspace/temp_dir',\
    "ST_reward_weights": {'spatial': 1,'temporal': 1,},\
    "scale_factor": 0.18215,\
    "decoder_bs": 8,\
    "use_fp16": True,\
    "data_align_method": None,\
    }
    from easydict import EasyDict
    cfg = EasyDict(cfg)

    rewardmodel = DiffRewardModel(reward_type = "HPSv2", 
                 cfg = cfg,
                 autoencoder = None,
                 segments = 8,
                 step_thresold = 300,
